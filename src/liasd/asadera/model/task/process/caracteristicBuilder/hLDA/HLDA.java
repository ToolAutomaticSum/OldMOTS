...
public class HLDA extends AbstractCaracteristicBuilder  implements IndexBasedIn<WordIndex>, SentenceCaracteristicBasedOut, HldaCaracteristicBasedOut {
	...
	public HLDA(int id) throws SupportADNException {
		...
		listParameterOut.add(new ParametrizedType(NCRPNode[].class, Map.class, SentenceCaracteristicBasedOut.class));
		listParameterOut.add(new ParametrizedType(double[].class, SentenceModel.class, HldaCaracteristicBasedOut.class));
		listParameterOut.add(new ParametrizedType(double[].class, NCRPNode.class, HldaCaracteristicBasedOut.class));
	}
	...
	public void initADN() throws Exception {
		int tempNumLevels = Integer.parseInt(getModel().getProcessOption(id, Inference_HLDA_Parameter.numLevels.getName()));
		getCurrentProcess().getADN().putParameter(new Parameter<Integer>(Inference_HLDA_Parameter.numLevels.getName(), tempNumLevels));
		getCurrentProcess().getADN().getParameter(Integer.class, Inference_HLDA_Parameter.numLevels.getName()).setMaxValue(2*tempNumLevels);
		getCurrentProcess().getADN().getParameter(Integer.class, Inference_HLDA_Parameter.numLevels.getName()).setMinValue(3);
		double tempAlpha = Double.parseDouble(getModel().getProcessOption(id, Inference_HLDA_Parameter.alpha.getName()));
		getCurrentProcess().getADN().putParameter(new Parameter<Double>(Inference_HLDA_Parameter.alpha.getName(), tempAlpha));
		getCurrentProcess().getADN().getParameter(Double.class, Inference_HLDA_Parameter.alpha.getName()).setMaxValue(2*tempAlpha);
		getCurrentProcess().getADN().getParameter(Double.class, Inference_HLDA_Parameter.alpha.getName()).setMinValue(1.0);	
		double tempGamma = Double.parseDouble(getModel().getProcessOption(id, Inference_HLDA_Parameter.gamma.getName()));
		getCurrentProcess().getADN().putParameter(new Parameter<Double>(Inference_HLDA_Parameter.gamma.getName(), tempGamma));
		getCurrentProcess().getADN().getParameter(Double.class, Inference_HLDA_Parameter.gamma.getName()).setMaxValue(2*tempGamma);
		getCurrentProcess().getADN().getParameter(Double.class, Inference_HLDA_Parameter.gamma.getName()).setMinValue(0.01);
		double tempEta = Double.parseDouble(getModel().getProcessOption(id, Inference_HLDA_Parameter.eta.getName()));
		getCurrentProcess().getADN().putParameter(new Parameter<Double>(Inference_HLDA_Parameter.eta.getName(), tempEta));
		getCurrentProcess().getADN().getParameter(Double.class, Inference_HLDA_Parameter.eta.getName()).setMaxValue(2*tempEta);
		getCurrentProcess().getADN().getParameter(Double.class, Inference_HLDA_Parameter.eta.getName()).setMinValue(0.03);	
	...
	public void processCaracteristics(List<Corpus> listCorpus) throws Exception {
		...
		for (int tokenId=0; tokenId<indexWord.size(); tokenId++)
			indexWord.get(tokenId).setWeight((indexWord.get(tokenId).getWeight()+1)/(indexWord.size()+summWeigth));
	...
	public boolean isOutCompatible(ParametrizedMethod compatibleMethod) {
		return (compatibleMethod.getParameterTypeIn().contains(new ParametrizedType(NCRPNode[].class, Map.class, SentenceCaracteristicBasedIn.class))
			&& (compatibleMethod.getParameterTypeIn().contains(new ParametrizedType(double[].class, SentenceModel.class, HldaCaracteristicBasedIn.class))
			|| compatibleMethod.getParameterTypeIn().contains(new ParametrizedType(double[].class, NCRPNode.class, HldaCaracteristicBasedIn.class))));
	}

	@Override
	public void setCompatibility(ParametrizedMethod compMethod) {
		((SentenceCaracteristicBasedIn) compMethod).setCaracterisics(sentenceCaracteristic);
		if (compMethod.getParameterTypeIn().contains(new ParametrizedType(double[].class, SentenceModel.class, HldaCaracteristicBasedIn.class)))
			((HldaCaracteristicBasedIn) compMethod).setSentenceLevelDistribution(sentenceLevelDistribution);
		if (compMethod.getParameterTypeIn().contains(new ParametrizedType(double[].class, NCRPNode.class, HldaCaracteristicBasedIn.class)))
