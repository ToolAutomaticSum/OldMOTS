...
public class Randoms extends java.util.Random {
	...
	/**
	 * Return a random BitSet with "size" bits, each having probability p of being
	 * true.
	 */
	...
	/**
	 * Return a random double in the range 0 to 1, inclusive, uniformly sampled from
	 * that range. The mean of this distribution is 0.5. The variance is 1/12.
	 */
	...
	/**
	 * Return a random double in the range a to b, inclusive, uniformly sampled from
	 * that range. The mean of this distribution is (b-a)/2. The variance is
	 * (b-a)^2/12
	 */
	...
	/**
	 * draw a single sample from (unnormalized) multinomial "a", with normalizing
	 * factor "sum".
	 */
	...
	/**
	 * Return a random double drawn from a Gamma distribution with mean alpha and
	 * variance 1.0.
	 */
	...
	/**
	 * Return a random double drawn from a Gamma distribution with mean alpha*beta
	 ...
	 * alpha*beta+lamba and variance alpha*beta^2. Note that this means the pdf is:
	 * <code>frac{ x^{alpha-1} exp(-x/beta) }{ beta^alpha Gamma(alpha) }</code> in
	 * other words, beta is a "scale" parameter. An alternative parameterization
	 * would use 1/beta, the "rate" parameter.
	 */
	public synchronized double nextGamma(double alpha, double beta, double lambda) {
		...
		if (alpha < 1) {
			...
			/*
			 * // Old version, uses time linear in alpha double y = -Math.log (nextUniform
			 * ()); while (nextUniform () > Math.pow (y * Math.exp (1 - y), alpha - 1)) y =
			 * -Math.log (nextUniform ()); gamma = alpha * y;
			 */
		}
	...
	/**
	 * Return a random double drawn from an Exponential distribution with mean beta
	 * and variance beta^2.
	 */
	...
	/**
	 * Return a random double drawn from an Chi-squarted distribution with mean 1
	 * and variance 2. Equivalent to nextChiSq(1)
	 */
	...
	/**
	 * Return a random double drawn from an Chi-squared distribution with mean df
	 * and variance 2*df.
	 */
	...
	/**
	 * Return a random double drawn from a Beta distribution with mean a/(a+b) and
