...
public class JSBigramUniqueProbScorer extends GeneticIndividualScorer{
	...
	public JSBigramUniqueProbScorer(HashMap <GeneticIndividualScorer, Double> scorers, Corpus corpus, HashMap<Integer, String> hashMapWord, InvertedIndex invertedIndex, Dictionnary index, Double divWeight, Double delta, Double firstSentenceConceptsFactor, Integer window, Double fsc_factor) {
	...
	private void computeSourceDistribution ()
	{
		...
		//{
			//for (SentenceModel p : doc.getSentenceModels())
	...
	public double computeScore(GeneticIndividual gi) {
		...
		this.smoothing = new DirichletUniqueProbSmoothing(2, this.delta, summDistrib.size(), gi.getGenes(), this.index, this.sourceDistribution, this.firstSentencesConcepts, this.firstSentenceConceptsFactor);
