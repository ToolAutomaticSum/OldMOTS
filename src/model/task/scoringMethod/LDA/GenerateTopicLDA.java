...
import model.task.summarizeMethod.LinkedSentenceLDA.SentenceComparator;
import textModeling.SentenceModel;
import textModeling.cluster.TopicLDA;
import tools.Tools;

public class GenerateTopicLDA extends AbstractScoringMethod implements LdaBasedIn, TopicLdaBasedOut{
	...
	public GenerateTopicLDA(int id) {
		super(id);
	}
	...
	public static List<SentenceModel> generateLinkedSentenceLDASummary(Map<SentenceModel, double[]> sentenceCaracteristic, double[] averageVector, List<TopicLDA> listTopic, int sizeSummary, int limitSentenceByTopic) throws VectorDimensionException {
		...
		for (int i = 0; i < listTopic.size(); i++) {
			if (listTopic.get(i).getScoreCorpus() != 0) {
				...
				comparator.setSentenceCaracteristic(sentenceCaracteristic);
	...
	public void computeScores() throws Exception {
		double bestScoreCorpus = 0;
		for (int k = 0; k<K; k++) { //topic loop
			List<SentenceModel> listSentenceTopic = new ArrayList<SentenceModel>();
			for (int j = 0; j<nbSentence; j++) {
				SentenceModel s = getCurrentProcess().getModel().getDocumentModels().getSentenceByID(j);
				double[] topicDistri = sentenceCaracteristic.get(s);
				if (topicDistri[k] != 0)
					listSentenceTopic.add(s);
			}
			double scoreCorpus = 0.0;
			for (int j=0; j<this.theta.length;j++) { //Boucle sur les documents
				scoreCorpus += this.theta[j][k];
			}
			scoreCorpus/=this.theta.length;
			if (scoreCorpus>bestScoreCorpus) {
				bestTopic = k;
				bestScoreCorpus = scoreCorpus;
			}
			if (scoreCorpus != 0) // TODO à tester*/
				listTopic.add(new TopicLDA(k, listSentenceTopic, scoreCorpus));
		}
		Collections.sort(listTopic);
